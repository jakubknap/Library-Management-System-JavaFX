...
import pl.knap.libsma.api.UserDao;
...
public class UserDaoImpl implements UserDao {
    ...
    public void updateData(Update key, User user, String other) throws SQLException {
        switch (key) {
            case LOGIN -> {
                String newLogin = user.getLogin();
                DatabaseManager.executeUpdate(Update.LOGIN, newLogin, other);
            }

            case PASSWORD -> {
                String newPassword = user.getPassword();
                String login = user.getLogin();
                DatabaseManager.executeUpdate(Update.PASSWORD, newPassword, login);
            }

            case TYPE -> {
                String newType = user.getType();
                String login = user.getLogin();
                DatabaseManager.executeUpdate(Update.TYPE, newType, login);
            }
        }
    }
