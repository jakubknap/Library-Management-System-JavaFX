...
import pl.knap.libsma.api.BookDao;
...
public class BookDaoImpl implements BookDao {
    ...
    public ObservableList<Book> getBooksInfo(BookType key) throws SQLException {
    ...
    public void updateData(Update key, Book book, String other) throws SQLException {
        switch (key) {
            case TITLE -> {
                DatabaseManager.executeUpdate(Update.TITLE, book.getTitle(), book.getIsbn());
            }

            case AUTHOR_NAME -> {
                DatabaseManager.executeUpdate(Update.AUTHOR_NAME, book.getAuthorName(), book.getIsbn());
            }

            case AUTHOR_SURNAME -> {
                DatabaseManager.executeUpdate(Update.AUTHOR_SURNAME, book.getAuthorSurname(), book.getIsbn());
            }

            case CATEGORY -> {
                DatabaseManager.executeUpdate(Update.CATEGORY, book.getCategory(), book.getIsbn());
            }

            case RELEASE_DATE -> {
                DatabaseManager.executeUpdate(Update.RELEASE_DATE, book.getReleaseDate(), book.getIsbn());
            }

            case ISBN -> {
                DatabaseManager.executeUpdate(Update.ISBN, book.getIsbn(), other);
            }

            case STATUS -> {
                DatabaseManager.executeUpdate(Update.STATUS, String.valueOf(book.getStatus()), book.getIsbn());
            }
        }
    }
